 ...
 * 2. 非阻塞队列
 *   2.1 chan<T> ch(N, discard_old): 具有N个缓冲区的实时队列,
 * 当N==0时队列长度为1。
...
class cv_t {
  ...
  public:
    ...
    void notify_one() {
        if (wait_count_ > 0) {
            wait_count_ =
                (wait_count_ > thread_count_ ? thread_count_ : wait_count_) - 1;
            cv_.notify_one();
        }
    }
    void notify_all() {
...
template <typename T> class queue_t {
  ...
  private:
    queue_t(size_t capacity, push_policy policy)
        : capacity_(capacity == 0 ? 1 : capacity), policy_(policy),
          cv_overflow_(capacity == 0 ? new std::condition_variable()
                                     : nullptr) {}
  ...
  public:
    ...
    // 入chan，支持move语义
    template <typename TR> bool push(TR &&data) {
        std::unique_lock<std::mutex> lock(mutex_);
        cv_push_.wait(lock, [&]() {
            return policy_ != push_policy::blocking || free_count() > 0 ||
                   closed_;
        });
        if (closed_) {
            return false;
        ...
        if (cv_overflow_ != nullptr) {
            ...
            cv_overflow_->wait(lock,
                               [&]() { return old != first_ || closed_; });
        }
    ...
    static std::shared_ptr<queue_t> make_queue(size_t capacity,
                                               push_policy policy) {
        if (policy != push_policy::blocking && capacity == 0) {
            capacity = 1;
        }
        const size_t size =
            sizeof(queue_t) + sizeof(T) * (capacity == 0 ? 1 : capacity);
        // 只有阻塞模式下才存在“溢出”等待区
        return std::shared_ptr<queue_t>(
            new (new char[size]) queue_t(capacity, policy), [](queue_t *q) {
                q->~queue_t();
                delete[] (char *)q;
            });
    }
  ...
  private:
    template <typename TR> bool push_thread_unsafe(TR &&d) {
        if (free_count() > 0) {
    ...
    size_t free_count() const { return first_ + capacity_ - new_; }
    bool is_empty() const { return first_ >= new_; }
    T &data(size_t pos) { return data_[pos % capacity_]; }
...
template <typename T> class chan {
  ...
  public:
    ...
    explicit chan(size_t concurrent_shift, size_t capacity,
                  push_policy policy = push_policy::blocking) {
    ...
    // 入chan，支持move语义
    template <typename TR> bool operator<<(TR &&data) {
        unsigned int index =
            data_->push_.fetch_add(1, std::memory_order_acq_rel);
    ...
    bool is_closed() const { return data_->queue_[0]->is_closed(); }

    // 出chan
    template <typename TR> bool operator>>(TR &d) {
        unsigned int index =
            data_->pop_.fetch_add(1, std::memory_order_acq_rel);
        return data_->queue_[index % length()]->pop(
            [&d](T &&target) { d = std::forward<T>(target); });
    }
    ...
    std::unique_ptr<T> pop() {
        unsigned int index =
            data_->pop_.fetch_add(1, std::memory_order_acq_rel);
        std::unique_ptr<T> d;
        data_->queue_[index % length()]->pop(
            [&d](T &&target) { d.reset(new T(std::forward<T>(target))); });
        return d;
    }

  private:
    size_t length() const { return data_->queue_.size(); }
};
